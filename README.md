...
## How to install
### Static
The files for RustJS are hosted on here and on <a href="https://rustjs.netlify.app/">the website</a> (hosted on Netlify), so to install RustJS, all you need to do is:<br>
&emsp;**1.** Add a `<script>` tag inside your HTML file. Place it before any runner files to avoid scope errors.<br>
&emsp;**2.** Set the source of the script to the latest version of RustJS, so all you need to do is set the version to `latest`.
      <br>&emsp;&emsp;&emsp;*NOTE: If you rely on a feature that becomes deprecated, it's recommended that you use a static version instead.*
```html
